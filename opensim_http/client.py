...
class ClientSubmit(object):
    ...
    def _parse_server_error_or_raise_for_status(self, resp):
        ...
        if resp.status_code != 200 and "message" in j:  # descriptive message from server side
            raise ServerErrorSubmit(message=j["message"], status_code=resp.status_code)
        resp.raise_for_status()
        return j
...
class ServerErrorSubmit(Exception):
