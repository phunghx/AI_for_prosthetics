...
class nnagent(object):
    ...
    train_multiplier=1,
        ...
        #self.rpm = rpm(self.memSize,self.batch_size,self.totalStep) # 1M history, total step = 100000
    ...
    def play(self,env,max_steps=-1,realtime=False,noise_level=0.): # play 1 episode
        ...
        while True and steps <= max_steps:
            ...
            if self.is_continuous and False:
            ...
            # o2, r1,
            try:
                observation, reward, done, _info = env.step([round(a,10) for a in action_out]) # take long time
                #print(reward)				
            except Exception as e:
            ...
            if self.training == True:
                ...
                if (current_step % self.frequence) == 0:
                     self.train(verbose=2 if steps==1 else 0,current_step=current_step)
                if (self.global_step % self.memSize) == 0:	self.rpm.rebalance()
            self.lock.release()
        ...
        if self.training == True:
          ...
          for t in episode_memory:
            if np.random.uniform()>0.5:
                  self.global_step +=1
                  self.feed_one(t)
          self.plotter.pushys([total_reward,noise_level,1.0*self.rpm.size()/self.memSize])
    ...
    def save_weights(self, episode):
        ...
        for name in networks:
            ...
            network.save_weights('./models/ddpg_'+name+str(episode) +'.npz')
...
if __name__=='__main__':
    ...
    def newpool():
        global epl,show_sim
        tepl = epl
        epl = eipool(36,showfirst=show_sim)
        del tepl
    def refarm():
        global farmer
        del farmer
    ...
    def playonce(nl):
        env = epl.acq_env()
        agent.play(env,realtime=False,max_steps=10000,noise_level=nl)
        epl.rel_env(env)
    def play_ignore(nl):
        import threading as th
        t = th.Thread(target=playonce,args=(nl,),daemon=True)
        t.start()
        # ignore and return.
    ...
    def up():
        ...
        while True:
            ...
            #exploration_noise = noise_source.one((agent.outputdims,),noise_level)
            action = agent.act(proc_observation)
            [observation, reward, done, info] = client.env_step(
                [float(round(a,10)) for a in action],
                True
            )
            stepno+=1
            total_reward+=reward
